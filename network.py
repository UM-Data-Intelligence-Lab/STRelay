...
class Flashback(nn.Module):
    ...
    def __init__(self, input_size, user_count, hidden_size, f_t, f_s, rnn_factory, lambda_loc, lambda_user, use_weight,
                 graph, spatial_graph, friend_graph, use_graph_user, use_spatial_graph, interact_graph, config=None):
        super().__init__()
        ...
        self.STRelay = STRelay(config)
        self.config = config

        emb_num = 2
        if self.config.STRelay:
            emb_num += 2
        self.fc = nn.Linear(emb_num * hidden_size, input_size)
        self.temporal_fc = nn.Linear(emb_num * hidden_size, config.temporal_intervals+1) 
        self.spatial_fc = nn.Linear(emb_num * hidden_size, config.spatial_intervals+1) 

    def forward(self, x, t, t_slot, s, y_t, y_t_slot, y_s, y_timeintervals, y_distances, active_user):
        ...
        out, _ = self.rnn(x_emb)  # (seq_len, user_len, hidden_size)
        ...
        t_linear = None
        if self.config.STRelay:
            temporal_context, spatial_contetxt = self.STRelay(x, t_slot, active_user)
            out_pu = torch.cat([out_pu, temporal_context, spatial_contetxt], dim=-1)
            t_linear = self.temporal_fc(out_pu)
            d_linear = self.spatial_fc(out_pu)
        y_linear = self.fc(out_pu)
        return y_linear, t_linear, d_linear
