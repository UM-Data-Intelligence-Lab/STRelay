...
setting.poi_count = poi_loader.locations()
setting.user_count = poi_loader.user_count()
...
trainer = FlashbackTrainer(setting.lambda_t, setting.lambda_s, setting.lambda_loc, setting.lambda_user,
                           ...
                           setting.use_spatial_graph, interact_graph)  
...
for e in range(setting.epochs):  # 100
    ...
    for i, (x, t, t_slot, s, y, y_t, y_t_slot, y_s, reset_h, y_timeintervals, y_distances, active_users) in enumerate(dataloader):
        ...
        active_users = active_users.squeeze().to(setting.device)
        y_timeintervals = y_timeintervals.squeeze().to(setting.device)
        y_distances = y_distances.squeeze().to(setting.device)
        optimizer.zero_grad()
        loss = trainer.loss(x, t, t_slot, s, y, y_t,
                            y_t_slot, y_s, y_timeintervals, y_distances, active_users)
