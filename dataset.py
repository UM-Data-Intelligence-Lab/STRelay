...
class PoiDataset(Dataset):
    ...
    def __init__(self, users, times, time_slots, coords, locs, sequence_length, batch_size, split, usage, loc_count,
        ...
        # self.sequences_lbl_distances = []
        # self.sequences_lbl_timeintervals = []
    ...
    def __getitem__(self, idx):
        ...
        y_timeintervals = ((y_t - t)/3600).long()
        y_timeintervals = torch.where(y_timeintervals > 24, 24, y_timeintervals)
        y_distances = haversine(y_s[:, :, 0], y_s[:, :, 1], s[:, :, 0], s[:, :, 1]).long()
        y_distances = torch.where(y_distances > 30, 30, y_distances)
        return x, t, t_slot, s, y, y_t, y_t_slot, y_s, reset_h, y_timeintervals, y_distances, torch.tensor(self.active_users)
